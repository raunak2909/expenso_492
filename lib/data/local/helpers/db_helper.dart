...
class DbHelper {
  ...
  Future<Database> openDB() async {
    ...
    return await openDatabase(dbPath, onCreate: (db ,version){
      ...
      db.execute(" create table $TABLE_USER ( $COLUMN_USER_ID integer primary key autoincrement, $COLUMN_USER_NAME text, $COLUMN_USER_MOBILE_NO text, $COLUMN_USER_EMAIL text, $COLUMN_USER_PASS text, $COLUMN_USER_CREATED_AT text, $COLUMN_USER_BAL real, $COLUMN_USER_BUDGET real )");
      db.execute(" create table $TABLE_EXPENSE ( $COLUMN_EXPENSE_ID integer primary key autoincrement, $COLUMN_USER_ID integer, $COLUMN_EXPENSE_TITLE text, $COLUMN_EXPENSE_REMARK text, $COLUMN_EXPENSE_AMT real, $COLUMN_EXPENSE_CAT_ID integer, $COLUMN_EXPENSE_TYPE integer, $COLUMN_EXPENSE_CREATED_AT text )");
  ...
  Future<bool> isEmailAlreadyExists({required String email}) async {
    Database db = await initDB();
    List<Map<String, dynamic>> result = await db.query(TABLE_USER, where: "$COLUMN_USER_EMAIL = ?", whereArgs: [email]);
